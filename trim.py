...
if __name__ == '__main__':
    ...
    for file in files:
        ...
        if (not os.path.exists('trim_bk')):
            os.mkdir('trim_bk')
        
        # 既に重複するファイル名が存在するなら名前に現在時刻を付加してから保存
        if (not os.path.isfile('trim_bk/'+img_name)):
            src.save('trim_bk/'+img_name, quality=75)
        else:
            date = str( datetime.now().strftime("%Y-%m-%d %H-%M-%S") )
            src.save('trim_bk/'+img_prefix+'_'+date+img_suffix, quality=75)
