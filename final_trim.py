...
if __name__ == '__main__':
    ...
    for file in files:
        ...
        cur = src.crop((margin, margin, src_width-margin, src_height-margin))
    
        # バックアップ用フォルダが存在しないなら作成
        if (not os.path.exists('final_trim_bk')):
            os.mkdir('final_trim_bk')
            
        # 既に重複するファイル名が存在するなら名前に現在時刻を付加してから保存
        if (not os.path.isfile('final_trim_bk/'+img_name)):
            src.save('final_trim_bk/'+img_name, quality=75)
        else:
            date = str( datetime.now().strftime("%Y-%m-%d %H-%M-%S") )
            src.save('final_trim_bk/'+img_prefix+'_'+date+img_suffix, quality=75)
